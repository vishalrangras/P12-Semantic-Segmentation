...
def load_vgg(sess, vgg_path):
	...
	# Some variables holding names of different layers of VGG16 model
	...
	# tf.get_default_graph() will load the default graph of the model which we loaded via vgg_path
    graph = tf.get_default_graph()
	
	# graph.get_tensor_by_name() will get specific tensors by their name in below lines of code
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
	...
	# Running a 1x1 convolution on Layer 7 of VGG16
	...
	# Applying a deconvolution on Layer 7 for Upsampling
	...
	output_4 = tf.layers.conv2d_transpose(skip_1, num_classes, 4, 2, padding='same', kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
	
	# Running a 1x1 convolution on Layer 3 of VGG16
	...
	# Skip connection connecting Upscaled output of Layer 4 and 1x1 Convolution output of Layer 3
	skip_2 = tf.add(output_4, conv_1x1_3)
	
	# Applying a deconvolution on Layer 3 output after the skip implementation for Upsampling
	output_3 = tf.layers.conv2d_transpose(skip_2, num_classes, 16, 8, padding='same', kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
	...
	for epoch in epochs:
		...
		for image, label in get_batches_fn(batch_size):
			_, loss = session.run([train_op, cross_entropy_loss], feed_dict={input_image: image, correct_label: label, keep_prob: 0.6, learning_rate: 0.0001})
...
def run():
    ...
    with tf.Session() as sess:
		...
		nn_output = layer(layer3_out, layer4_out, layer7_out, num_classes)
